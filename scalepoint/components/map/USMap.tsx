              ...
              {points.map(({ m, x, y }) => {
                ...
                return (
                  <g key={m.id} transform={`translate(${x},${y})`}>
                    {active && (
                      <circle r={r + 4} fill="none" stroke="#C6A15E" strokeWidth={1.5} className="motion-safe:animate-halo" />
                    )}
                    <circle
                      r={r}
                      fill={active ? '#C6A15E' : '#2F8F72'}
                    ...
                    />
                    {active && (() => {
                      // Flip the label to the left of the node when it's near the
                      // right edge, so the text never gets clipped by the map bounds.
                      const nearRight = x > WIDTH - 150;
                      return (
                        <text
                          x={nearRight ? -(r + 8) : r + 8}
                          y={4}
