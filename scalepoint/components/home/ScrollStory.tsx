    ...
    <section ref={wrapRef} className="relative bg-carbon on-dark" style={{ height: `${100 + n * STEP_VH}vh` }}>
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <div className="shell relative grid h-full grid-cols-1 items-center gap-8 lg:grid-cols-[1.05fr_0.95fr]">
          {/* 3D stage */}
          <div className="relative flex h-[52vh] items-center justify-center lg:h-[70vh]" style={{ perspective: '1100px' }}>
            {/* rotating platform glow */}
            <div aria-hidden className="absolute bottom-[18%] h-24 w-64 rounded-[100%]" style={{ background: `radial-gradient(closest-side, rgba(${accent},0.4), transparent)`, filter: 'blur(6px)' }} />
            {/* central figure */}
            <div className="absolute z-20 h-[46%] w-auto" style={{ aspectRatio: '160 / 200', transform: `rotateY(${Math.sin(progress * Math.PI * 2) * 8}deg)` }}>
              <Figure accent={accent} />
            </div>
            {/* orbit ring */}
            <div className="absolute inset-0" style={{ transformStyle: 'preserve-3d', transform: `rotateX(8deg) rotateY(${ringRotation}deg)` }}>
              {features.map((f, i) => {
                const world = ((i * step + ringRotation) % 360 + 360) % 360; // 0..360, 0 = front
                const rad = (world * Math.PI) / 180;
                const front = (Math.cos(rad) + 1) / 2; // 1 front, 0 back
                ...
                return (
                  ...
                  >
                    <div
                      className={cn('flex flex-col items-center gap-1.5 transition-transform duration-300')}
                      style={{ transform: `scale(${0.8 + front * (isActive ? 0.55 : 0.3)})` }}
                    >
                      <span
                        className="flex h-14 w-14 items-center justify-center rounded-2xl border backdrop-blur-sm transition-colors duration-300"
                        style={{
                          borderColor: isActive ? `rgb(${accent})` : 'rgba(244,238,224,0.18)',
                          background: isActive ? `rgba(${accent},0.22)` : 'rgba(244,238,224,0.05)',
                          boxShadow: isActive ? `0 0 28px rgba(${accent},0.5)` : 'none',
                        }}
                      >
                        <svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke={isActive ? `rgb(${accent})` : '#F4EEE0'} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d={f.icon} /></svg>
                      </span>
                      <span className={cn('rounded-full px-2.5 py-0.5 text-[11px] font-semibold tracking-wide transition-colors', isActive ? 'text-mineral' : 'text-mineral/60')}>
                        {f.label}
                      </span>
                    </div>
                  </div>
