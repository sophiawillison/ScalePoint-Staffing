...
export function EmployerHome() {
  return (
    <>
      {/* 1 — Hero (immersive, distinct from executive) */}
      <section className="relative overflow-hidden bg-carbon on-dark">
        <div aria-hidden className="pointer-events-none absolute inset-0"
          style={{ background: 'radial-gradient(70% 60% at 20% 10%, rgba(121,208,221,0.14), transparent 65%)' }} />
        <div className="shell relative flex min-h-[86vh] flex-col justify-center pb-24 pt-36">
          <Reveal><p className="eyebrow eyebrow--light">Executive Search · Talent Intelligence</p></Reveal>
          <Reveal delay={80}>
            <h1 className="mt-5 max-w-4xl font-serif text-display font-medium text-mineral">
              Find the leaders who can move your business forward.
            </h1>
          </Reveal>
          <Reveal delay={160}>
            <p className="mt-6 max-w-xl text-[19px] leading-relaxed text-mineral/70">
              Mandate-led executive search across the United States. We map the market, assess leadership
              evidence, and present a deliberately small, qualified shortlist.
            </p>
          </Reveal>
          <Reveal delay={240}>
            <div className="mt-9 flex flex-wrap gap-3">
              <ButtonLink href="/employer-search" variant="light" arrow>Submit a Hiring Requirement</ButtonLink>
              <ButtonLink href="/talent-intelligence" variant="ghost" className="!text-mineral hover:!text-cyan">
      ...
      {/* 3 — Leadership Across America (employer map w/ lenses) */}
      <Section dark>
      ...
      <Section dark className="!pt-0">
        <Reveal><SectionHead dark eyebrow="How We Work"
          ...
          intro="Five stages from market discovery to a selective shortlist. Step through each to see what it involves." /></Reveal>
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={t} className="flex items-center gap-3 rounded-xl border border-mist bg-mineral px-4 py-3 text-[15px] text-ink">
                  <span aria-hidden className="text-brass">—</span>{t}
                </li>
              ))}
            </ul>
          </Reveal>
      ...
      <Section>
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <div className="mt-8 flex justify-center gap-3">
            <ButtonLink href="/employer-search" variant="primary" arrow>Brief Our Executive Search Team</ButtonLink>
