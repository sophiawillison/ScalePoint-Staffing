...
export function HeroVideo({
  ...
  return (
    <div className={cn('absolute inset-0 overflow-hidden', className)} aria-hidden>
      ...
      {/* accent tint sits behind the scrim so text contrast stays consistent */}
      {accent && <div className="absolute inset-0" style={{ background: accent }} />}
      {/* readability scrim, guarantees overlaid text and nav stay legible over any footage */}
      <div className="absolute inset-0 bg-carbon/60" />
      <div className={cn('absolute inset-0 bg-gradient-to-b from-carbon/35 via-carbon/25 to-carbon', overlay && '')} />
      {/* gentle vignette */}
      <div className="absolute inset-0 bg-[radial-gradient(120%_120%_at_50%_-10%,transparent_45%,rgba(8,32,26,0.55)_100%)]" />
    </div>
  );
}
