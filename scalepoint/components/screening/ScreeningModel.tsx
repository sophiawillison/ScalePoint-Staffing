...
export function ScreeningModel() {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
      {/* stage rail — a genuine sequence, so numbering is appropriate */}
    ...
    return (
      <svg viewBox="0 0 320 170" className="w-full" role="img" aria-label="Search universe narrowing toward a mandate">
        {Array.from({ length: 26 }).map((_, i) => {
          ...
          return <line key={i} x1={160} y1={85} x2={x} y2={y} stroke="#1E3247" strokeWidth={1} />;
        })}
        {Array.from({ length: 26 }).map((_, i) => {
          const a = (i / 26) * Math.PI * 2;
          const rad = 52 + (i % 3) * 20;
          const x = 160 + Math.cos(a) * rad;
          const y = 85 + Math.sin(a) * (rad * 0.5);
          return <circle key={i} cx={x} cy={y} r={i % 5 === 0 ? 3.2 : 1.9} fill={i % 5 === 0 ? '#8EA9FF' : '#3A5674'} />;
        })}
        <circle cx={160} cy={85} r={6.5} fill="#79D0DD" />
      </svg>
    );
  }
  if (kind === 'evidence') {
  ...
  if (kind === 'radar') {
    ...
    return (
      <svg viewBox="0 0 190 160" className="mx-auto w-[190px]" role="img" aria-label="Calibration across mandate dimensions (no numeric scoring)">
        {[26, 46, 66].map((r) => <circle key={r} cx={95} cy={80} r={r} fill="none" stroke="#1E3247" strokeWidth={1} />)}
